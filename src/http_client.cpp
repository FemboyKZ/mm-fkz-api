...
public:
  HttpRequestContext(HttpResponseHandler handler, void *userData)
      : m_handler(handler), m_userData(userData),
...
private:
  void OnCompleted(HTTPRequestCompleted_t *pResult, bool bIOFailure) {
    ...
    if (!bIOFailure && pResult->m_bRequestSuccessful) {
      ...
      if (http) {
        uint32 size = 0;
        if (http->GetHTTPResponseBodySize(pResult->m_hRequest, &size) &&
            size > 0) {
          body.resize(size);
          http->GetHTTPResponseBodyData(pResult->m_hRequest, (uint8 *)&body[0],
                                        size);
    ...
    if (m_handler)
      m_handler(success, statusCode, body.c_str(), (uint32)body.size(),
                m_userData);

    // Deregister and self-destruct.
    // Steam's CCallResult marks itself invalid before invoking this handler,
    // so deleting here is safe.
...
void HttpClient::OnContextFinished(HttpRequestContext *ctx) {
  m_pending.erase(std::remove(m_pending.begin(), m_pending.end(), ctx),
                  m_pending.end());
}
...
bool HttpClient::Request(const char *method, const char *url, const char *body,
                         uint32 timeoutSec, HttpResponseHandler handler,
                         void *userData) {
  ...
  if (body && body[0] != '\0' &&
      (eMethod == k_EHTTPMethodPOST || eMethod == k_EHTTPMethodPUT ||
       eMethod == k_EHTTPMethodPATCH)) {
    http->SetHTTPRequestRawPostBody(req, "application/json", (uint8 *)body,
                                    (uint32)strlen(body));
  }
