...
static std::string BuildPluginsJson()
{
	...
	ISmmPluginManager *plmgr = (ISmmPluginManager *)g_SMAPI->MetaFactory(MMIFACE_PLMANAGER, &ret, NULL);
	...
	for (PluginId id = Pl_MinId; id <= 256 && misses < 16; id++)
	{
		...
		arr += std::string("\"status\":\"") + (status == Pl_Running ? "running" : "paused") + "\"}";
	}
...
void ResolveIpPort(char *ip, int ipLen, int &port)
{
	...
	else
	{
		...
		if (cvHostIp.IsValidRef())
		{
			...
			snprintf(ip, ipLen, "%d.%d.%d.%d", (hostip >> 24) & 0xFF, (hostip >> 16) & 0xFF, (hostip >> 8) & 0xFF, hostip & 0xFF);
		}
		else
		{
