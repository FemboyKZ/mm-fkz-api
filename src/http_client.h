...
typedef void (*HttpResponseHandler)(bool success, int statusCode, const char *body, uint32 bodyLen, void *userData);
...
public:
	...
	// Returns true if the request was dispatched (the handler fires later)
	// and false if it could not be sent (the handler is NOT called in that case).
	bool Request(const char *method, const char *url, const char *body, uint32 timeoutSec, HttpResponseHandler handler, void *userData);
